...
public interface ProdutoRepository extends JpaRepository<Produto, Integer> {
    ...
    @Query("SELECT p FROM Produto p ORDER BY COALESCE(p.precoDesconto, p.preco) ASC")
    List<Produto> findAllOrderByPrecoAsc();

    @Query("SELECT p FROM Produto p ORDER BY COALESCE(p.precoDesconto, p.preco) DESC")
